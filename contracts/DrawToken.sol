...
contract DrawToken is Token {
    ...
    constructor (address deployer) public{
        owner = deployer;
        balances[deployer] = 1000000000000000000;   //배포자 주소에 이더 저장시키기 balances가 각 주소의 이더량.
        totalSupply = 1000000000000000000;                        // 총 이더의 수
        _tokenName = "DRAW TOKEN";                                   // 토큰 이름
        _decimalUnits = 18;                            // Amount of decimals for display purposes
        _tokenSymbol = "DRAW";                               // Set the symbol for display purposes
    }
    ...
    function getHanyangBToken(address _to, uint amount) external returns(bool success){
    ...
    function useToken(address user)external returns(bool) {
        if(balances[user]>=1){
            balances[user]--;
            return true;
        }else{
    ...
    function ownerMoney() external view returns(uint){
        return houseEther;//배포자가 충전한 ether의 금액을 보여줌
    }

    function getDrawToken(address _to, uint amount) external returns(bool success){
            if (houseEther >= amount && amount > 0) {
            //하우스로부터 돈을 받는 스마트 컨트랙트이다.
            houseEther -= amount;
            balances[_to] += amount;
